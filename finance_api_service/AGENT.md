...
Finance statement sync defaults to `120` quarterly periods plus `30` annual
periods (`quarter=0`) so the first run loads historical statements for all
eligible companies. Do not reduce those defaults unless the sync strategy
changes.
