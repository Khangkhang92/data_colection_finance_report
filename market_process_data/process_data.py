...
def get_history_data_by_symbol(db: ScopedSession, symbol_ticker: str):
    try:
        query = (
            select(HistoryDataProcessing)
            .where(HistoryDataProcessing.symbol_ticker == symbol_ticker)
            .order_by(HistoryDataProcessing.date.asc())
        )
            ...
            for item in next_20:
                if item.percent_change is not None:
                    if item.percent_change > 0:
                        ...
                        continuous_negative = 0
                        max_continuous_positive = max(
                            max_continuous_positive, continuous_positive
                        )

                    elif item.percent_change < 0:
                        continuous_negative -= 1
                        continuous_positive = 0
                        max_continuous_negative = min(
                            max_continuous_negative, continuous_negative
                        )
                    else:
                        continuous_positive = 0
                        continuous_negative = 0
