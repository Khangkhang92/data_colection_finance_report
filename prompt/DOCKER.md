...
cp .env.example .env
docker compose -f build/compose/docker-compose.yml up -d postgres postgrest redis api celery-worker celery-beat
```
...
```bash
docker compose -f build/compose/docker-compose.yml down -v
```
