...
- file cau hinh database nam trong `app/finance_report/.env`
...
- `finance-schema` va `finance_report` da duoc cai vao env
...
```bash
cd /home/khangst92/Documents/finance/data_colection_finance_report/finance_report
/home/khangst92/miniconda3/bin/conda run -n module1 python -m pip install -r requirements.txt
/home/khangst92/miniconda3/bin/conda run -n module1 python -m pip install -e .
```

Trong mono-repo local, `app/finance_report/requirements.txt` cai editable
`../finance_schema_lib`. Neu `finance-schema` da duoc publish rieng, co the dung
`app/finance_report/requirements.release.txt`.
...
```bash
export FINANCE_SCHEMA_ENV_FILE=/home/khangst92/Documents/finance/data_colection_finance_report/app/finance_report/.env
```
...
```bash
FINANCE_SCHEMA_ENV_FILE=/home/khangst92/Documents/finance/data_colection_finance_report/app/finance_report/.env \
...
- file `app/finance_report/.env` co ton tai khong
