...
PostgreSQL` trong `build/compose/docker-compose.yml`, Alembic migrations tu package
`finance-schema`, va backend trong `finance_report`.

## 1. Chuan bi moi truong

Tu thu muc `finance_report`:
...
```bash
docker compose -f build/compose/docker-compose.yml up -d postgres redis
...
`POSTGRES_DB` trong `build/compose/docker-compose.yml`.
...
```bash
docker compose -f build/compose/docker-compose.yml up -d api celery-worker celery-beat
```
