...
with ScopedSession() as session: 
    for period in periods:
        ...
        market_mentions = session.query(MarketMention).filter(MarketMention.date == today).all() 
        if market_mentions:
           market_mention_lookup = {mention.symbol_ticker: mention for mention in market_mentions}
